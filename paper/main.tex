...
Our work combines these ideas by using hard negatives with known attribute violations to provide graded supervision for standard dense-retrieval losses.
...
To generate conjunctive queries, we prompt Gemini 2.5 Flash\footnote{We manually reviewed and verified the generated annotations and iteratively refined the prompts. We selected Gemini 2.5 Flash as a cost-effective model for this annotation process.} with a matched positive and negative document pair, using the prompts of Appendix~\ref{prompt:attribute-extraction}, one variant for products and one for images.
...
We set the margin term to $d/V$\footnote{Preliminary experiments with several nonlinear margin functions indicated that the $d/V$ formulation yielded the best performance.} and apply the loss across all query--document pairs in the batch:
