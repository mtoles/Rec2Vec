...
Formally, following \citep{lu2025multiconir}, let query $q_k$ be a query composed of $k$ conditions $C=\{c_1, c_2, ..., c_k\}$ with $k \in \{1, ..., K\}$.
...
Under the assumptions of Theorem~\ref{thm:forward}, affine recoverability of attributes permits low Margin-MSE Attribute Hamming Loss.
...
Across all models 96 models, we find a strong linear relationship between recall on human versus synthetic queries ( Pearson $r \geq 0.83$, $R^2 \geq 0.70$).
At the same time, model ordering model ordering is moderately positive (Spearman $\rho = 0.47$) on text to strongly positive ($\rho = 0.75$) on images.
...
This cannot be explained simply by the lower average conditions on human queries because condition count is in fact positively correlated with recall (Figure~\ref{fig:infonce-condition-counts}).
...
\begin{figure}[t]
    ...
    \begin{minipage}[c]{0.27\linewidth}
        \centering
        \darkgreen{\input{figs/human_synthetic_correlations.tex}}
    \end{minipage}
    \caption{\darkgreen{Synthetic versus attribute-count-matched human recall for text (Recall@5, left) and images (Recall@20, centre), with correlation statistics at right.
    Each plot includes all 48 model--seed points; colours and markers follow Figure~\ref{fig:text-vs-image-recall}, and dashed lines show linear fits.}}
...
We present \textsc{gold} mining, a multi-modal hard negative mining algorithm that improves recall on conjunctive queries.
