...
To generate a contrasting text document, we sample a document uniformly at random from the top five candidates most similar to the original ESCI query. 
...
We include images and descriptions of both products in the prompt, instructing the model not to generate attributes that are not visible in the images.
...
Specifically, each document will violate exactly distance $d$ conditions in the query, where $1 \leq d \leq \min(k, 10)$.
We term this strategy \textsc{Gold} mining.
Thus, \textsc{Gold} mining produces (query, positive, negative, distance) tuples, where $d$ is consumed by the \textsc{Gold} reweighting objective described in Section \red{tbd}.
...
To study the NV-Retriever baseline, we perform a search across NV-Retriever mining parameters described in Section \red{??}.
...
\paragraph{InfoNCE:}
Every document in a batch of $B$ triples is a candidate for every query: the $B$ positives and the $B$ negatives, the set $X$ of $2B$ candidates.
...
\begin{align}
    ...
    &= - \underbrace{\left(1-\frac{d}{V}\right)}_{\text{ours}}
    \log \frac{\exp\big(s\inner{f(q_i)}{f(x_i^+)}\big)}
    {\sum_{x\in X\setminus\{x_i^-\}}\exp\big(s\inner{f(q_i)}{f(x)}\big)}
    \notag\\
    &\quad - \underbrace{\frac{d}{V}}_{\text{ours}}
    \log \frac{\exp\big(s\inner{f(q_i)}{f(x_i^+)}\big)}
    {\sum_{x\in X}\exp\big(s\inner{f(q_i)}{f(x)}\big)} .
\end{align}
We fix $s=20$ and choose $V\ge K^*$ so that $d/V\in[0,1]$.
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{(a) Text versus image retrieval recall on the test split.
    Arrows indicate that one training strategy builds on another.
    Unfilled shapes indicate baselines.
    (b--e) Text and (f--i) image recall (vertical axis) versus positive/hard-negative win rate (horizontal axis). Axis scales held constant across plots. Across loss functions, \textsc{Gold} mining consistently improves win rate while \textsc{Gold} reweighting improves recall at the expense of win rate; $n=3$ per point.}
...
\textsc{Gold} mining + reweighting, however, generally performs better than NV-Retriever across modalities, improving recall in the two strongest loss functions on text and all four on images.
Overall, the strongest strategy for text recall is InfoNCE with \textsc{Gold} mining + reweighting.
...
We observe that \textsc{Gold} mining increases hard-negative win rate in seven of eight loss function--modality pairs compared to Baseline.
Applying \textsc{Gold} reweighting improves synthetic recall in seven of eight loss function--modality pairs at the expense of hard-negative win rate.
...
To validate the use of synthetic queries, we evaluate the same models, all trained on the Main train split, on both the synthetic Main test set and human-written queries.
...
When combined with \textsc{Gold} mining, reweighting beats the strongest baseline in most cases and vanilla training in all.
...
% REVIEW: Current mining_report.json files use top_k=10 or 100 for both modalities, not 1000 for text.
\textcolor{red}{We mine the train split only, with \texttt{e5-mistral-7b-instruct} for text and \texttt{clip-ViT-L-14} for images, retrieving $1000$ candidates per text query and $100$ per image query before the positive-aware filter; if no candidate survives, we retain the lowest-scoring retrieved non-positive candidate.} % FACTCHECK: top_k was swept over {10, 100} in both modalities; selected top_k=10 for text, 100 for images (mining_report.json).
\red{We grid-search the filter's relative margin $m \in \{0.025, 0.05, 0.1, 0.2\}$ against a survivor offset $s \in \{0, 10\}$, taking the $(s+1)$-th survivor rather than the first.} % FACTCHECK: grid is top_k in {10, 100} x margin: k=10 with {none, 0.05, 0.1, 0.2}, k=100 with {0.05, 0.1, 0.2, 0.3}; no survivor offset (skip_survivors=0); selected text k=10/m=0.1, image k=100/m=0.3.
\red{If fewer than $s+1$ candidates survive, we take the last survivor.} % FACTCHECK: no survivor offset was used, so this rule never applies.
