% @Claude and @codex NEVER edit this without explicit instruction to do so
...
One limitation in existing training paradigms is the binary training objective; hard negatives that subtly conflict with the query are considered equally incorrect as easy, obviously irrelevant negatives.
...
Applying both achieves equal or better recall than random and mined negatives across all tested loss functions and both text and image modalities.
We attribute this improvement to the following property: under suitable query and comparison coverage assumptions, low loss on the \textsc{gold} Margin-MSE objective implies approximate affine recoverability of attributes, a property not satisfiable by the standard objective.
...
% \darkgreen{Under explicit norm and scale assumptions, the hard-negative Margin-MSE objective admits low-loss cosine query representations formed from signed sums of the vector parts of affine attribute probes.}
% \darkgreen{Under a coverage assumption, low count loss on a query pair differing in one condition implies approximate affine recovery of the deleted attribute, whereas zero binary Margin-MSE loss on the same query family need not.}
...
\citet{papenmeier2020modern} find that the length of users' free-form text descriptions of their desired product include on average 29 words and 3.83 attributes. 
...
Unfortunately, publicly available product retrieval datasets such as \cite{??} lack conjunctive queries.
Instead, datasets like the popular ECSI \cite{reddy2022shopping} focus on providing hard negatives---similar pairs of similar documents, only one of which is valid.
...
Thus, we identify dense retrieval as understudied across three axes: data, methods, and evaluations.
...
Leveraging the structured attribute lists allows us to calculate the number of query conditions violated by the hard negative, which we use the \textsc{gold} reweighting training objective.
...
Our results show that \textsc{gold} mining improves recall across all four loss functions, especially on the text domain. 
In most cases, this improvement exceeds that of the state-of-the-art NV-Retreiver mining algorithm. 
...
\cite{wang2024improvingtextembeddingslarge, lee2024geckoversatiletextembeddings} distill embeddings from LLMs.
...
Traditional Margin-MSE loss \citep{hofstatter2020improving} \darkgreen{using cosine similarity between normalized representations} is defined as
...
\begin{equation}
    \label{eq:trad-mse}
    \mathcal{L}(q, x^+, x^-) = \Big( \operatorname{cos}(f(q), f(x^+)) - \operatorname{cos}(f(q), f(x^-)) - y \Big)^2 .
\end{equation}
...
\darkgreen{Retrieval scores use cosine similarity, $\operatorname{cos}(a,b)=a^\top b/(\|a\|\|b\|)$, while $\inner{a}{b}=a^\top b$ denotes the Euclidean inner product.}
\darkgreen{Because $f$ returns unit-norm embeddings, $\operatorname{cos}(f(q),f(x))=\inner{f(q)}{f(x)}$, so a difference of cosine scores equals $\inner{f(q)}{f(x^+)-f(x^-)}$.}
\darkgreen{The difference vector and the probe vectors below are not normalized.}
...
\begin{equation}
\label{eq:hamming}
    \mathcal{L_\text{MSE}}(q_k, x^+, x^-) = \Big( \operatorname{cos}(f(q_k), f(x^+)) - \operatorname{cos}(f(q_k), f(x^-)) - \underbrace{\tfrac{d}{V}}_{\text{ours}} \Big)^2
\end{equation}
...
\darkgreen{We say a feature is $(\epsilon, S)$-affinely recovered by a probe $(b_i,\beta_i)\in\mathbb{R}^d\times\mathbb{R}$ if}
...
\darkgreen{Under the assumptions of Theorem~\ref{thm:forward}, affine recoverability of attributes permits low Margin-MSE Attribute Hamming Loss.}
\darkgreen{Conversely, uniformly low loss across all positive--negative comparisons for two queries differing by one condition, each with nonempty positive and negative sets, implies approximate affine recovery of that condition's attribute (Theorem~\ref{thm:back}).}
\darkgreen{Binary-target Margin-MSE provides no such guarantee: zero loss can coexist with an unrecoverable attribute (Proposition~\ref{prop:binary}).}
\darkgreen{Finally, for documents with at most $k'$ active attributes from a vocabulary of size $m$, queries with at most $k$ conditions, and $V\ge kk'$, we construct unit document and query embeddings in dimension $d=O\!\left(\frac{k'^2}{\epsilon^2}\log m\right)$ that affinely recover every attribute with error below $\epsilon$ and achieve loss below $(2k\epsilon/V)^2$ on every valid comparison (Corollary~\ref{cor:capacity}).}
...
\begin{figure}
    ...
    \darkgreen{The query is the normalized sum of the vector parts $b_i$ of its attributes' affine probes, whose biases are zero in this example.}
...
\darkgreen{Theorem~\ref{thm:forward} constructs cosine queries from affine attribute probes under explicit norm and scale assumptions.}
\darkgreen{Theorem~\ref{thm:back} shows that, under complete comparison coverage, low count loss on a query pair differing in one condition implies affine recovery of the deleted attribute.}
...
\end{equation}
\darkgreen{This extends the linear accessibility definition of \citet[Section~1.1]{garg2026many}, whose probes have no bias.}
...
\begin{theorem}[\darkgreen{Affine recoverability permits low loss}]
\label{thm:forward}
...
\begin{theorem}[\darkgreen{Low loss implies affine recoverability}]
\label{thm:back}
...
\darkgreen{Then $G_C-G_D$ affinely recovers $h_c$, and hence its underlying attribute, with error $4V\delta$; the probe for $h_c$ is}
\begin{equation}
\label{eq:app-probe}
...
Using our attribute Hamming loss, on the other hand, $(0,0)$ and $(0,1)$ have different targets because they violate two conditions and one condition of $C$, respectively.
\darkgreen{Per Theorem~\ref{thm:back}, sufficiently small loss on these two queries implies that $z_2$ is approximately recoverable by an affine probe, even without a query for $z_2$ alone.}
...
For fixed document sparsity $k'$ and attribute recovery error $\epsilon$, the sufficient dimension is logarithmic in $m$.
\darkgreen{The padding preserves attribute recovery with zero bias, a special case of affine recovery, although the complete embedding need not be linear in $z$.}
The condition $V\ge kk'$ is sufficient and is not claimed to be necessary; the corollary is an existence result under this scale condition, not a guarantee about the embeddings learned by training.
