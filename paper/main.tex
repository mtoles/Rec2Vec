...
It then follows that user query behavior may suffer from a chicken-and-egg problem; users do not use complex queries because they expect models to fail, while practitioners to do not optimize for complex queries because they do not appear in user data.
...
We fine-tune \texttt{all-mpnet-base-v2} on text and \texttt{clip-ViT-B-32} on images, both as distributed by Sentence-Transformers \citep{reimers-gurevych-2019-sentence}; the text tower encodes queries in both modalities, and we normalise every representation to unit length.
We optimise with AdamW \citep{loshchilov2017decoupled} at learning rate $2 \times 10^{-5}$, a cosine schedule warming up over the first 10\% of steps, and bf16, training each condition three times and report the mean over those trials.
...
I.e., \textsc{gold} mining improves accuracy more on hard negatives while \textsc{gold} reweighting trades accuracy on hard negatives for accuracy on easy negatives.
This is the expected result of penalizing errors in hard negatives less than easy negatives in the loss function.
...
Datasets containing more hard negatives per query will benefit may benefit more from \textsc{gold} mining.
Applications requiring maximum recall will benefit more from \textsc{gold} reweighting.
\red{linear regression on V ablation?}

We see in Figure~\ref{fig:infonce-condition-counts} that InfoNCE accuracy for both \textsc{gold} mined + reweighted and random baseline accuracy is positively correlated with the number of positive (required) attributes on both modes. 
However, accuracy appears to be largely uncorrelated with the the number of negative (forbidden) attributes.
...
The lack of correlation with forbidden attributes suggests that overall, embedding models neither induce false positives out of documents containing those attributes nor contribute to to magnifying positive documents in the way that required attributes do.
...
\begin{figure}[t]
    ...
    \begin{subfigure}{\linewidth}
        \centering
        \scalebox{1}[0.9]{\includegraphics[width=\linewidth]{figs/text_vs_image_recall.pdf}}
        \caption{\darkgreen{Text Recall@5 versus image Recall@20.}}
    ...
    \begin{subfigure}{\linewidth}
        ...
        \caption{\darkgreen{Recall versus positive/hard-negative win rate for text (left) and images (right).}}
        \label{fig:recall-vs-winrate}
    \end{subfigure}
    \caption{\darkgreen{Retrieval recall and hard-negative win rate on the test split ($n=3$ per point).
...
\begin{figure}[t]
    ...
    \caption{\darkgreen{InfoNCE recall by query condition count.
    Text uses Recall@5 and images use Recall@20, on a shared colour scale.
    Both axes show counts from 0 to 10; grey cells contain no test queries.}}
